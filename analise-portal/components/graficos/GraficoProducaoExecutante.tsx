...
interface GraficoProducaoExecutanteProps {
...
// Função para extrair abreviação do nome (ex: "Selmira Maria Pamplona" -> "SMP")
const extrairAbreviacao = (nome: string): string => {
  const palavras = nome.trim().split(/\s+/)
  if (palavras.length === 0) return ''
  
  // Pegar a primeira letra de cada palavra (máximo 4 palavras)
...
export default function GraficoProducaoExecutante({ filtros }: GraficoProducaoExecutanteProps) {
  ...
  const buscarDados = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/indicadores/producao-executante?${params.toString()}`)
      if (response.ok) {
  ...
  const COR_CUMPRIDO = '#10b981' // verde
  const COR_COM_PARECER = '#f59e0b' // amarelo/âmbar

  return (
    <Card>
      <CardHeader>
        <CardTitle>Visão de Produção por Executante</CardTitle>
        {total > 0 && (
            ...
            >
              ...
              <Tooltip
                formatter={(value: number, name: string) => {
                  const label = name === 'cumprido' ? 'Cumprido' : 'Cumprido com parecer'
              ...
              <Legend
                ...
                formatter={(value: string) => {
                  if (value === 'cumprido') return 'Cumprido'
                  if (value === 'comParecer') return 'Cumprido com parecer'
                  return value
                }}
              />
              <Bar
              ...
              <Bar
                dataKey="comParecer"
                stackId="a"
                fill={COR_COM_PARECER}
                name="comParecer"
              >
                {/* Label no final da barra mostrando apenas "Cumprido com parecer" */}
                <LabelList
                  dataKey="comParecer"
                  position="right"
                  offset={10}
